...
public class CritterModel {
    ...
    public void add(int number, Class<? extends Critter> critter) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            info.put(next, new PrivateData(new Point(x, y), d, 0, new LinkedList<Boolean>()));
        }
    ...
    private Info getInfo(PrivateData data, Class original) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if(neighbors[i] == Critter.Neighbor.OTHER || neighbors[i] == Critter.Neighbor.SAME){
        ...
        return new Info(neighbors, data.direction, neighborDirections, data.moveScore);
    }
    ...
    public void update() {
        ...
        // Assign a sortValue for each critter based on the
        //    moveScore (number of hops in last 10 turns)
        // The sort value is a number between 0 and 19
        //   a moveScore of 0 gets to choose randomly between 0 and 9
        //   and a movescore of 10 gets to choose randomly between 0 and 19
        Random r = new Random();
        for(int i = 0; i < list.length; i++){
            PrivateData data = info.get(list[i]);
            data.sortValue = r.nextInt(data.moveScore + 10);
        }

        //preshuffle so matching sortValues are shuffled
        Collections.shuffle(Arrays.asList(list));
        // sort by the sortValue
        Arrays.sort(list, new Comparator() {
                public int compare(Object x, Object y) {
                    return info.get(y).sortValue - info.get(x).sortValue;
                }
            });


        // get all actions decisions from the critter at the start of the turn
        Critter.Action actions[] = new Critter.Action[list.length];
        for (int i = 0; i < list.length; i++) {
            Critter next = (Critter)list[i];
            PrivateData data = info.get(next);
            actions[i] = next.getMove(getInfo(data, next.getClass()));
        }
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            if (move == Critter.Action.LEFT) {
                ...
                if (inBounds(p2) && grid[p2.x][p2.y] == null) {
                    ...
                    grid[p.x][p.y] = null;
                    data.p = p2;
                    data.addMoveWasHop(true);
                    locked.add(next); //successful hop locks a critter from being infected
    ...
    private class PrivateData {
        ...
        public Queue<Boolean> lastMovesAreHops;
        public int sortValue;
        public Color color;
        public String string;

        public PrivateData(Point p, Critter.Direction d, int moveScore, Queue<Boolean> lastMovesAreHops) {
            this.p = p;
            this.direction = d;
            this.moveScore = moveScore;
            this.lastMovesAreHops = lastMovesAreHops;
        }

        // This takes a boolean to say whether the new move was a hop, updates the memory
        // of the last 10 moves (lastMovesAreHops) and updates the moveScore
        public void addMoveWasHop(boolean wasHop){
            lastMovesAreHops.add(wasHop);
            if(wasHop){
               moveScore++;
            }
            if(lastMovesAreHops.size() > 10){
               if(lastMovesAreHops.remove()){
                  moveScore--;
               }
            }
        }
    ...
    private static class Info implements CritterInfo {
        ...
        public Info(Critter.Neighbor[] neighbors, Critter.Direction d,
                    Critter.Direction[] neighborDirections, int moveScore) {
